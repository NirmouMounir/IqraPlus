...
data class SurahReciter(override val id: Int, override val name: String, val count: Int, val riwayah: String, val server: URL, val suras: String): Reciter(id, name)
...
suspend fun getReciters(type: ReciterType): List<Reciter> = withContext(Dispatchers.IO) {
    ...
    for (reciterJson in reciterJsonArray) {
        ...
        val reciter: Reciter = if (type === ReciterType.SURAH) {
            SurahReciter(jsonObj.get("id").asInt, jsonObj.get("name").asString, jsonObj.get("count").asInt, jsonObj.get("rewaya").asString, URL(jsonObj.get("Server").asString), jsonObj.get("suras").asString)
        } else {
